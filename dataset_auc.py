...
class Casia_dataset(Dataset): 
    def __init__(self, root, mode, transform = None, transform_depth = None, attack = None):
        ...
        if self.mode == 'train':
            ...
            #                      ('HR' not in name and (name.split("_")[2][-1] == '3' or name.split("_")[2][-1] == '4' or name.split("_")[2][-1] == '5' or name.split("_")[2][-1] == '6')))]
            # elif self.attack =="real":
            #     self.pathname = [name for name in self.pathname if (('HR' in name and (name.split("_")[3][0] == '1')) or 
            #                      ('HR' not in name and (name.split("_")[2][-1] == '1' or name.split("_")[2][-1] == '2')))]
            # elif self.attack == "replay":
            #     self.pathname = [name for name in self.pathname if (('HR' in name and (name.split("_")[3][0] == '4')) or 
            #                      ('HR' not in name and (name.split("_")[2][-1] == '7' or name.split("_")[2][-1] == '8')))]
            if self.attack == "print":
                self.pathname = [name for name in self.pathname if (('HR' in name and (name.split(".")[0][-1] == '2' or name.split(".")[0][-1] == '3')) or 
                                 ('HR' not in name and (name.split(".")[0][-1] == '3' or name.split(".")[0][-1] == '4' or name.split(".")[0][-1] == '5' or name.split(".")[0][-1] == '6')))]
            elif self.attack =="real":
                self.pathname = [name for name in self.pathname if (('HR' in name and (name.split(".")[0][-1] == '1')) or 
                                 ('HR' not in name and (name.split(".")[0][-1] == '1' or name.split(".")[0][-1] == '2')))]
            elif self.attack == "replay":
                self.pathname = [name for name in self.pathname if (('HR' in name and (name.split(".")[0][-1] == '4')) or 
                                 ('HR' not in name and (name.split(".")[0][-1] == '7' or name.split(".")[0][-1] == '8')))]
        else:
            self.pathname = self.pathname      
        self.len = len(self.pathname) #全部pic個數
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            if 'HR' in self.pathname[index]:
                if self.pathname[index].split(".")[0][-1] == '1': #real pic
                    label = int(0)
                    triplet_label = int(0)
                elif self.pathname[index].split(".")[0][-1] == '2' or self.pathname[index].split(".")[0][-1] == '3': #print
                    label = int(1)
                    triplet_label = int(1)
                elif self.pathname[index].split(".")[0][-1] == '4': #replay
                    label = int(1)
                    triplet_label = int(2)
            else:
                if self.pathname[index].split(".")[0][-1] == '1' or self.pathname[index].split(".")[0][-1] == '2': #real pic
                    label = int(0)
                    triplet_label = int(0)
                elif self.pathname[index].split(".")[0][-1] == '3' or self.pathname[index].split(".")[0][-1] == '4' or self.pathname[index].split(".")[0][-1] == '5' or self.pathname[index].split(".")[0][-1] == '6': #print
                    label = int(1)
                    triplet_label = int(1)
                elif self.pathname[index].split(".")[0][-1] == '7' or self.pathname[index].split(".")[0][-1] == '8': #replay
                    label = int(1)
                    triplet_label = int(2)
        else: #test,dev
...
class CelebA_Spoof_dataset(Dataset): 
    def __init__(self, root, mode, transform = None, attack = None):
        self.common_path = root
        self.mode = mode # train, eval, test
        self.transform = transform
        self.attack = attack
        self.filename = sorted(os.listdir(self.common_path))
        self.pathname = [number for number in self.filename if 'png' in number]
        if self.mode == 'train':
            self.pathname = self.pathname
        else:
            self.pathname = self.pathname  
        self.len = len(self.pathname) #全部pic個數
        print('CELEBA_SPOOF {} {}: {}'.format(self.mode, self.attack, self.len))
...
>>>>>>> bc3cf4c84a58cbab1d947b8d7489482beb209058
    ...
    idiap_train_real_dataset_new = Idiap_dataset(os.path.join(path, 'ReplayAttack/replayattack-train/real/crop_frame/crop_face'), 'train', transform = transform, transform_depth = transform_depth, attack = "real")
    idiap_train_print_fixed_dataset_new = Idiap_dataset(os.path.join(path, 'ReplayAttack/replayattack-train/attack/fixed/crop_frame/crop_face'), 'train', transform = transform, transform_depth = transform_depth, attack = "print")
    idiap_train_replay_fixed_dataset_new = Idiap_dataset(os.path.join(path, 'ReplayAttack/replayattack-train/attack/fixed/crop_frame/crop_face'), 'train', transform = transform, transform_depth = transform_depth, attack = "replay")
    idiap_train_print_hand_dataset_new = Idiap_dataset(os.path.join(path, 'ReplayAttack/replayattack-train/attack/hand/crop_frame/crop_face'), 'train', transform = transform, transform_depth = transform_depth, attack = "print")
    idiap_train_replay_hand_dataset_new = Idiap_dataset(os.path.join(path, 'ReplayAttack/replayattack-train/attack/hand/crop_frame/crop_face'), 'train', transform = transform, transform_depth = transform_depth, attack = "replay")
    ...
    oulu_train_real_dataset_new = Oulu_dataset(os.path.join(path, 'Oulu_NPU/Train_files/crop_frame/crop_face'), 'train', transform = transform, transform_depth = transform_depth, attack='real')
    oulu_train_print_dataset_new = Oulu_dataset(os.path.join(path, 'Oulu_NPU/Train_files/crop_frame/crop_face'), 'train', transform = transform, transform_depth = transform_depth, attack = "print")
    oulu_train_replay_dataset_new = Oulu_dataset(os.path.join(path, 'Oulu_NPU/Train_files/crop_frame/crop_face'), 'train', transform = transform, transform_depth = transform_depth, attack = "replay")
    ...
    casia_train_real_dataset_new = Casia_dataset(os.path.join(path, 'CASIA_faceAntisp/train_release/crop_frame/crop_face'), 'train', transform = transform, transform_depth = transform_depth, attack='real')
    casia_train_print_dataset_new = Casia_dataset(os.path.join(path, 'CASIA_faceAntisp/train_release/crop_frame/crop_face'), 'train', transform = transform, transform_depth = transform_depth, attack='print')
    casia_train_replay_dataset_new = Casia_dataset(os.path.join(path, 'CASIA_faceAntisp/train_release/crop_frame/crop_face'), 'train', transform = transform, transform_depth = transform_depth, attack='replay')
...
def choose_dataset_2domain(path, target_domain, img_size, depth_size):
    ...
    test_dataset_c = casia_test_dataset
    test_dataset_o = oulu_test_dataset
    ...
    return  test_dataset_c, test_dataset_o, domain1_real_dataset, domain1_print_dataset, domain1_replay_dataset, \
            domain2_real_dataset, domain2_print_dataset, domain2_replay_dataset