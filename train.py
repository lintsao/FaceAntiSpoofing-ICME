...
def train(path='./', dataset_path='./dataset', target_domain='oulu', number_folder='8', img_size=256, depth_size=64, batch_size=8, batch_triplet=4, lr=0.0003, n_epoch=100):
    same_seeds(0)
    ...
    opt_spoof_classify_scheduler = optim.lr_scheduler.MultiStepLR(opt_spoof_classify, milestones=[30, 70, 90], gamma=0.3)
    ...
    for epoch in range(n_epoch):
        ...
        for i, ((d1_data, d1_depth, d1_label), (d2_data, d2_depth, d2_label), (d3_data, d3_depth, d3_label)) in enumerate(zip(domain1_loader, domain2_loader, domain3_loader)):
            ...
            # opt_domain_a_scheduler.zero_grad()
            # opt_domain_b_scheduler.zero_grad()
            # opt_domain_c_scheduler.zero_grad()
            # opt_domain_classify_scheduler.zero_grad()
            ...
            # opt_shared_content_scheduler.zero_grad() 
            # opt_shared_spoof_scheduler.zero_grad() 
            ...
            # opt_shared_spoof_scheduler.zero_grad() 
            # opt_spoof_classify_scheduler.zero_grad() 
            spoof_classify.eval()
