...
def train_celeba_baseline(args):
    ...
    shared_spoof_path, spoof_classify_path, shared_content_path, domain1_encoder_path, domain2_encoder_path, domain3_encoder_path, \
    ...
    opt_shared_feature = optim.AdamW(shared_feature.parameters(), lr = args.lr)
    ...
    opt_shared_feature_scheduler = optim.lr_scheduler.MultiStepLR(opt_shared_feature, milestones=[5,25,30, 70, 90,120,150,200,250,300,400], gamma=0.9)
    ...
    for epoch in range(args.n_epoch):
        ...
        for i, ((d1_data, d1_depth, d1_label), (d2_data, d2_depth, d2_label), (d3_data, d3_depth, d3_label)) in enumerate(zip(domain1_loader, domain2_loader, domain3_loader)):
            ...
            ###SHARED###
            mixed_data = shared_feature(mix_data)
            ...
            #spoof部分
            mixed_data = shared_feature(mix_data).to(device)

            ###Extract feature###
            spoof_feature = shared_spoof(mixed_data)

            # spoof_domain_logit = softmax(domain_classify(spoof_feature))
            domain_grl_spoof_loss =  0# gamma*class_criterion_re(spoof_domain_logit, mixed_label_domain) 
            e_domain_grl_spoof_loss += domain_grl_spoof_loss

            #content部分
            # content_domain_logit = softmax(domain_classify(content_feature))
            domain_grl_content_loss = 0#gamma*class_criterion_re(content_domain_logit, mixed_label_domain) 
            ...
            #spoof_class_loss = class_criterion(spoof_logit, mixed_label)
            ...
            spoof_grl_content_loss =0# gamma*class_criterion_re(content_logit, mixed_label_re)
            spoof_grl_domain_loss = 0#gamma*class_criterion_re(domain_logit, mixed_label_re)
            ...
            print("\r {}/{} domain_class_loss:{:.5f}, domain_grl_spoof_loss={:.5f}, domain_grl_content_loss={:.5f}, spoof_class_loss={:.4f}, spoof_grl_content_loss={:.5f}, spoof_grl_domain_loss={:.5f}, depth_loss = {:.5f}".format(
                    i+1, len_dataloader, 0,0,0, spoof_class_loss.item(), 
                    0, 0,0), end = "")
