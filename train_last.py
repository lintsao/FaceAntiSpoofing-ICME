...
def train(path='./', dataset_path='./dataset', target_domain='oulu', number_folder='2', img_size=256, depth_size=64, batch_size=8, batch_triplet=4, lr=0.0003, n_epoch=100):
    same_seeds(0)
    ...
    alpha, beta, gamma, theta = 0.01, 0.0001, 0.001, 0.01  # beta: spoof, gamma: grl, theta: recon
    ...
    for epoch in range(n_epoch):
        ...
        for i, ((d1_data, d1_depth, d1_label), (d2_data, d2_depth, d2_label), (d3_data, d3_depth, d3_label)) in enumerate(zip(domain1_loader, domain2_loader, domain3_loader)):
            ...
            d1_data = d1_data.expand(len(d1_data), 3, img_size , img_size)[:len_data].to(device)
            d2_data = d2_data.expand(len(d2_data), 3, img_size , img_size)[:len_data].to(device)
            d3_data = d3_data.expand(len(d3_data), 3, img_size , img_size)[:len_data].to(device)
            ...
            spoof_classify.eval() # 不要動

            ###Step 3 : 讓Domain Classify GRL回spoof和content###
            #spoof部分
            ...
            loss = domain_grl_spoof_loss + domain_grl_content_loss + spoof_grl_content_loss + spoof_grl_domain_loss
            loss.backward()
            opt_shared_spoof.step()
            ...
            depth_loss = alpha*mse_loss(depth_recon, mixed_depth)
            ...
            recon_loss = alpha*err_sim1 + alpha*err_sim3 + alpha*err_sim5
            e_recon_loss += recon_loss

            loss = depth_loss + recon_loss
            loss.backward()
            opt_decode.step()
            opt_depth.step()
            ...
            print("\r {}/{} domain_class_loss:{:.5f}, domain_grl_spoof_loss={:.5f}, domain_grl_content_loss={:.5f}, spoof_class_loss={:.4f}, spoof_grl_content_loss={:.5f}, spoof_grl_domain_loss={:.5f}, recon_loss = {:.5f}, depth_loss = {:.5f}, swap_loss = {:.5f}".format(
                    ...
                    spoof_grl_content_loss.item(), spoof_grl_domain_loss.item(), recon_loss.item(), depth_loss.item(), swap_loss), end = "")
        ...
        print("{}/{} e_domain_class_loss:{:.5f}, e_domain_grl_spoof_loss={:.5f}, e_domain_grl_content_loss={:.5f}, e_spoof_class_loss={:.4f}, e_spoof_grl_content_loss={:.5f}, e_spoof_grl_domain_loss={:.5f}, e_recon_loss = {:.5f}, e_depth_loss = {:.5f}, e_swap_loss = {:.5f}".format(
                ...
                e_spoof_grl_content_loss.item(), e_spoof_grl_domain_loss.item(), e_recon_loss.item(), e_depth_loss.item(), e_swap_loss))
